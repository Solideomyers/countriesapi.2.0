...
export const Nav = () => {
  ...
  return (
    <header className="flex items-center justify-between xl:justify-start w-full px-8 py-4 h-[10vh] z-50 mb-8">
      ...
      <nav
        className={`fixed py-8 bg-white bg-opacity-70 rounded-2xl backdrop-blur-3xl backdrop-filter w-[70%] md:w-[40%] xl:w-full h-full ${
          show ? 'left-0' : '-left-full'
        } top-0 xl:static flex-1 flex flex-col xl:flex-row items-center justify-center  gap-10 transition-all duration-500`}
      >
          {!ubication ? (<Search />) : ''}
        <Link to="/home">
          <a href="#" className="font-nunito-light text-lg font-bold px-4 py-[1.2rem] transition-all ease-in-out duration-500 delay-75 rounded-xl hover:bg-primary hover:bg-opacity-90 hover:text-white hover:drop-shadow-[4px_4px_5px_black]">
            Home
          </a>
        </Link>
        <Link to="/form">
      ...
      <Button flex={null} text={null} linked={null} disabled={null} size={'2xl'} styles={" xl:hidden text-2xl p-2 border-none"} onClick={() => setShow(!show)} background={null} color={'primary'} icon={show ? <MdClose /> : <HiMenuAlt3 />} />
    </header>
  )
}
