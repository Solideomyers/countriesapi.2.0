...
export const getByName = createAsyncThunk('countries/getByName', async name => {
  const response = await axios.get(`/countries/?name=${name}`)
  return response.data
})

export const getById = createAsyncThunk('countries/getById', async id => {
...
const countriesSlice = createSlice({
  ...
  reducers: {
    getByContinent(state, action) {
      ...
      state.sorting =
        action.payload === 'all' ? [...copyCountry] : [...continentFiltered]
    },
